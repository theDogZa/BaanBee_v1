...
class ItemsController extends Controller
{
    ...
    public function index(Request $request)
	{
		...
		if (@$request->all()) {
			...
			$results = $this->_search($request, $results);
			...
			// 	//$results = $results->sortable()->paginate(config('core.paginate'));
		...
		$compact = $this->_list_select_hasMany();
	...
	public function _ajaxGetItemList(Request $request,$id = null)
	{
		$compact = array();
		//$search = $request->input('search');

		if($id){
			$results = App\Models\Warehouse_item::where('warehouse_id', '=', $id)->get();
		}else{
		...
		$compact['results'] = $results;
		$compact['arrShowField'] = $this->arrShowFieldIndex;
		return view('items.item_list', $compact);
	}

	public function __select_items(){
	...
	private function _search($request, $results)
	{
	...
	private function _list_select_hasMany()
	{
		$return = array();
		...
		$return['Item_category'] = $Item_categorys;
		...
		$Item_colors = App\Models\Item_color::where('id', '!=', null)
			...
			->pluck('name', 'id')
			->toArray();
		$return['Item_color'] = $Item_colors;

		$Item_units = App\Models\Item_unit::where('id', '!=', null)
			->orderBy('id')
			->pluck('unit_name', 'id')
			->toArray();
		$return['Item_unit'] = $Item_units;
