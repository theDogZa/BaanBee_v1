    ...
    <form action="{{ url('/item_units'.( isset($item_unit) ? "/" . $item_unit->id : "")) }}" method="POST" class="form-horizontal form-process" id="form" >
        ...
        {!! (!empty( $arrShowFieldFrom['unit_name'] ))?       
            '<div class="'.config('core.layout.form.add.box').'">'.
                Form::label('unit_name', __("item_unit.label_unit_name").' *', array('class' => config("core.layout.form.add.label")))
        ...
        {!! (!empty( $arrShowFieldFrom['unit_desc'] ))?
            '<div class="'.config('core.layout.form.add.box').'">'.
                Form::label('unit_desc', __("item_unit.label_unit_desc").' ', array('class' => config("core.layout.form.add.label")))
                .'<div class="'.config('core.layout.form.add.input').'">'.               
                Form::textarea('unit_desc', @$item_unit['unit_desc'], array('rows'=> config("core.layout.input.textarea.rows"),'placeholder'=>__("item_unit.placeholder_unit_desc"),'class' => 'form-control text-editor','','', 'data-parsley-error-message' =>  __("validation.required", ['attribute' => __("item_unit.label_unit_desc")]) ))
                .'</div>
            </div>'
        : '' !!}
        ...
        {!! (!empty( $arrShowFieldFrom['active'] ))?
            '<div class="'.config('core.layout.form.add.box').'">'.
                Form::label('active', __("item_unit.label_active").' ', array('class' => config("core.layout.form.add.label")))
                .'<div class="'.config('core.layout.form.add.input').'">'.             
